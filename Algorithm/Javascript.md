...
- yield 只能在生成器函数（generator）里使用，生成器用 function* 声明。
- yield 是“暂停点”：执行到 yield 时函数暂停，并把右侧的值作为迭代结果返回；下一次用 next() 恢- 复执行。
- next(value) 会把这个 value 作为“上一个 yield 表达式的返回值”传回生成器，从而实现双向通信。
- yield* 用来“委托”到另一个可迭代对象或生成器，相当于把它的所有产出逐个 yield 出来。


```js
function* gen() {
  console.log('start');
  const a = yield 10;      // 暂停，返回 { value: 10, done: false }
  console.log('got a =', a);
  const b = yield 20;      // 暂停，返回 { value: 20, done: false }
  console.log('got b =', b);
  return 30;               // 结束，返回 { value: 30, done: true }
}

const g = gen();

console.log(g.next());        // { value: 10, done: false }, 打印 "start"
console.log(g.next('X'));     // { value: 20, done: false }, 打印 "got a = X"
console.log(g.next('Y'));     // { value: 30, done: true },  打印 "got b = Y"

// 注意：for...of 不会遍历到 return 的最终值（done: true 的 value 被忽略）
```
    ...
    function* dfs(arr) {
        for (let i = 0; i < arr.length; i++) {
            if (Array.isArray(arr[i])) {
                yield* dfs(arr[i]); // 使用 yield* 委托给另一个生成器
            } else {
                yield arr[i];
            }
        }
...
// 测试
const generator = inorderTraversal([1, [2, 3], 4]);
