...
两个特殊场景，退化为RNN
- 当重置门($Z_t$)为1，候选隐状态($\tilde{H_i})$就是RNN的递推公式
- 当重置门($Z_t$)为1，更新门($Z_t$)为0，隐状态($H_{t_i}$)就是RNN的递推公式
...
def create_batch_dataset(lines, vocab, seq_length=100, batch_size=64):
    ...
    dataset_xy = data_generator.map(split_input_target)  # 将一个batch去除的一组字符，按split_input_target函数，分解成两个部分，这两部分一个是input，另一个是output，实际就是一对(input, output)
